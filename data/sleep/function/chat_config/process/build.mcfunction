...
data modify storage config_system:temp menu set value []

# Add title
# Line break
function sleep:chat_config/process/append/handle/line_break

$data modify storage config_system:temp menu append value {text:"$(title)",bold:true}
...
# Store options in temp storage
$data modify storage config_system:temp options set value $(options)

# pagination type + store data
data modify storage config_system:temp pagination set from storage config_system:temp options[-1]
data remove storage config_system:temp options[-1]
...
# copy of all options
data modify storage config_system:temp all_options set from storage config_system:temp options
...
# Display
$tellraw $(target) {nbt:"menu[]",storage:"config_system:temp",interpret:true,separator:""}